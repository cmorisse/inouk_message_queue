...
from odoo.addons.inouk_message_queue.api import (processor, 
                                                processor_method, 
                                                send_message)
from odoo.addons.inouk_message_queue.models.message_processor import IMQ_MESSAGE_PROCESSOR_TYPES
...
class IMQTestLauncher(models.Model):
    ...
    debug_mode = fields.Boolean(help="When checked, RCP code est executed synch"
                                     "ronously.")
    launch_result = fields.Text()
    process_result = fields.Text()
    message_group = fields.Char(help="Sent to SQS as MessageGroupId param for FIFO Queues.")
