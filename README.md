...
bin/start_odoo imq_worker --database $PGDATABASE --queues=default

# Process multiple queues with a glob pattern
bin/start_odoo imq_worker --database $PGDATABASE --queues="high_priority_*" --max-messages=1000

# Target a specific list of queues by name (regex alternation)
bin/start_odoo imq_worker --database $PGDATABASE --queues='(default|pack8s|healthcheck)'

# Run with monitoring enabled
bin/start_odoo imq_worker --database $PGDATABASE --queues=default --observability-port=9090
```
...
bin/start_odoo imq_worker --database $PGDATABASE --queues "mpy.*" --max-messages 100
...
bin/start_odoo imq_worker --database $PGDATABASE --queues default --message 49737
...
When `imq_worker` rejects bad CLI arguments (missing `--queues`, mutually exclusive flags, unknown queue pattern, etc.), it prints a clear error to **stderr** but the shell exit code is **always `0`**.

This is an upstream Odoo behavior: [`odoo/cli/command.py`](https://github.com/odoo/odoo/blob/19.0/odoo/cli/command.py) invokes `o.run(args)` without `sys.exit()`, so any `return N` from a `Command.run()` subclass is silently discarded. It affects every `odoo.cli.Command` (not just IMQ — also `odoo shell`, `odoo populate`, etc.).
...
cat /proc/$(pgrep -f 'imq_worker')/limits | grep "Max processes"
...
RestartSec=5s
LimitNPROC=2048
ExecStart=bin/start_odoo imq_worker --queues muppy --max-thread-delta 80 --observability-port 9000
```
...
ExecStart=bin/start_odoo imq_worker \
    --queues=default \
    --worker-name=imq-default \
    --metrics-export-mode=textfile \
    --textfile-dir=/var/lib/node_exporter/textfile
...
# Process the message
bin/start_odoo imq_worker --database $PGDATABASE --queues default --message $MESSAGE_ID --max-messages 1
...
bin/start_odoo imq_worker --database $PGDATABASE --queues default --max-messages 5 &
WORKER_PID=$!
...
2. **CLI Command Registration Test**: Confirms the `imq_worker` command is properly registered
...
bin/start_odoo help | grep imq_worker

# Test help system
bin/start_odoo imq_worker --help

# Test worker with non-existent queue (should exit gracefully)
bin/start_odoo imq_worker --database $PGDATABASE --queues test_queue --max-messages 1
```
...
bin/start_odoo imq_worker --database $PGDATABASE --queues default --max-rss-memory 512M

# Test with regex pattern
bin/start_odoo imq_worker --database $PGDATABASE --queues "mpy.*" --max-messages 10

# Test with observability (metrics and health checks)
bin/start_odoo imq_worker --database $PGDATABASE --queues default --observability-port 8080
```
